...
3. Go through each of the queue files one item at a time, for each business:
   ...
   3.2. If offering is available in the period: inspect its website, its google maps reviews (use `scripts/google-maps.ts`, which launches `${process.env.HOME}/code/scripts/src/google-maps.ts`), look it up on trip advisor and compile a high level summary of the offering in a standardized format, note the reviews and all the other requirements.
